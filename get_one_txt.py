...
# Beautiful Soup的用法（五）：select的使用   https://blog.csdn.net/weixin_40425640/article/details/79470617
# 一般使用BeautififulSoup解析得到的Soup文档可以使用find_all()、find()、select() 方法定位所需要的元素。
# find_all()是获得list列表、find() 是获得map一条数据。select() 是根据选择器可以获得多条也可以获得单条数据。
# 一般最常用的是find_all() 和 find() 两个参数。
# select 的功能跟find和find_all 一样用来选取特定的标签，它的选取规则依赖于css，我们把它叫做css选择器
...
def get_txt(txt_id):
    ...
    try:
        ...
        soups = BeautifulSoup(res.text, "html.parser")             #soup转化   美化html
        #获取小说题目
        txt['title'] = soups.select('#wrapper .box_con #maininfo #info h1')[0].text    #0  第几个所满足的
        ...
        # 中查找
        # 标签名为a，id
        # 为link2的tag，这样像树状一层一层的查找，在分析html结构是是非常常见的。层和层之间用空格分开。
        ...
        try:
            fo_err.write(('['+time.strftime('%Y-%m-%d %X', time.localtime())+"]：编号：" + '{0:0>8}   '.format(txt['id'])
                          + "小说名：《" + txt['title'] + "》 下载失败。\r\n").encode('UTF-8'))
            print('['+time.strftime('%Y-%m-%d %X', time.localtime())+"]：编号："+'{0:0>8}   '
                  .format(txt['id'])+  "小说名：《"+txt['title']+"》 下载失败。")
            os.rename('{0:0>8}'.format(txt['id']) + '-' + txt['title'] + '.txt.download',
                  '{0:0>8}'.format(txt['id']) + '-' + txt['title'] + '.txt.error')
        except:
            fo_err.write(('['+time.strftime('%Y-%m-%d %X', time.localtime())+"]：编号："+'{0:0>8}   '
                          .format(txt['id'])+"下载失败。\r\n").encode('UTF-8'))
            print('['+time.strftime('%Y-%m-%d %X', time.localtime())+"]：编号："+'{0:0>8}   '.format(txt['id'])+"下载失败。")
        finally: #关闭文件
            fo_err.close()

def  bool_getHTMLText(url):     #get our url and judge if it can work normally;
    try:
